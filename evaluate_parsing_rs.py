#!/usr/bin/env python3
"""
Reference Parsing Evaluation Script (Rust-backed)

Compares PDF reference extraction against ground truth from .bib files,
identifies parsing failures, and helps improve the extraction pipeline.
...
Usage:
    ...
    python evaluate_parsing_rs.py /path/to/downloads -o report.md  # With output
    python evaluate_parsing_rs.py /path/to/downloads --json    # JSON output
    python evaluate_parsing_rs.py /path/to/downloads -v        # Verbose
    python evaluate_parsing_rs.py /path/to/downloads --limit 10    # Test mode
"""
...
def match_references(pdf_refs: list, bib_entries: list[BibEntry]) -> tuple[list[MatchResult], list[BibEntry]]:
    """Match PDF references to .bib entries.

    Args:
        pdf_refs: list of Reference objects from hallucinator bindings.
        bib_entries: list of BibEntry parsed from .bib files.
...
def compare_paper(pdf_path: str, tar_path: str, extractor: PdfExtractor,
                  verbose: bool = False, max_refs: int = 0) -> PaperReport:
    ...
    # Extract references from PDF using Rust bindings
    try:
        result = extractor.extract(pdf_path)
        pdf_refs = result.references
    ...
    if verbose:
        print(f"  Extracted {len(pdf_refs)} refs from PDF (raw: {skip_stats.total_raw}, "
              f"skipped: url={skip_stats.url_only}, short={skip_stats.short_title}, "
              f"no_authors={skip_stats.no_authors})")
...
def process_directory(downloads_dir: str, limit: Optional[int] = None,
    ...
    # Create a single extractor instance (reused across all papers)
    ...
    for pdf_path in pdf_files:
        ...
        report = compare_paper(str(pdf_path), str(tar_path), extractor,
                               verbose=verbose, max_refs=max_refs)
...
def generate_markdown_report(reports: list[PaperReport], stats: dict) -> str:
    ...
    lines = []
    lines.append("# Reference Parsing Evaluation Report (Rust backend)\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate reference parsing against .bib ground truth (Rust backend)"
    )
    parser.add_argument(
        'downloads_dir',
    ...
    print(f"Processing papers in: {args.downloads_dir} (using hallucinator-rs)")
    if args.offset:
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"Summary: {stats['total_papers']} papers (hallucinator-rs)")
