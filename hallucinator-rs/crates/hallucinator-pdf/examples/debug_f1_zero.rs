    ...
    for entry in fs::read_dir(corpus_dir)? {
        ...
        if path.extension().map_or(false, |e| e == "pdf") {
            if let Some(tarball) = find_matching_tarball(&path) {
                if let Ok(()) = check_paper(&path, &tarball, &config) {
                    checked += 1;
                    if checked >= limit {
                        break;
                    }
...
fn check_paper(pdf_path: &Path, tarball_path: &Path, config: &PdfParsingConfig) -> Result<()> {
    ...
    if let Some(result) = best {
        ...
        if f1 < 0.1 && !extracted_titles.is_empty() {
            ...
            println!("Strategy: {:?}, {} refs extracted, {} titles extracted",
                result.strategy, result.references.len(), extracted_titles.len());
            ...
            for et in extracted_titles.iter().take(5) {
                let best_match = ground_truth.iter()
                    .map(|gt| (gt, strsim::jaro_winkler(&normalize_title(et), &normalize_title(gt))))
