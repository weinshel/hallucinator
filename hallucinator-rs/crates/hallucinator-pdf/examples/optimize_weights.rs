...
use hallucinator_pdf::section::{find_references_section, segment_references_all_strategies, SegmentationStrategy};
use hallucinator_pdf::scoring::ScoringWeights;
...
fn main() -> Result<()> {
    ...
    let total_combinations = coverage_vals.len() * completeness_vals.len()
        * consistency_vals.len() * specificity_vals.len();
    let mut checked = 0;
        ...
        for &w_comp in &completeness_vals {
            for &w_cons in &consistency_vals {
                for &w_spec in &specificity_vals {
                    ...
                    if accuracy > best_accuracy || (accuracy == best_accuracy && avg_f1 > best_avg_f1) {
                        best_accuracy = accuracy;
                        best_avg_f1 = avg_f1;
                        best_weights = weights.clone();
                        eprintln!(
                            "[{}/{}] New best: acc={:.1}%, avgF1={:.3} | cov={:.2}, comp={:.2}, cons={:.2}, spec={:.2}, cnt={:.2}",
                            checked, total_combinations,
                            accuracy * 100.0, avg_f1,
                            w_cov, w_comp, w_cons, w_spec, w_count
                        );
                    }
                }
            }
    ...
    eprintln!();
    eprintln!("Train accuracy: {:.1}% (avg F1: {:.3})", best_accuracy * 100.0, best_avg_f1);
    eprintln!("Test accuracy:  {:.1}% (avg F1: {:.3})", test_accuracy * 100.0, test_avg_f1);
...
fn load_paper_data(pdf_path: &Path, tarball_path: &Path, config: &PdfParsingConfig) -> Result<PaperData> {
    ...
    for result in all_results {
        ...
        let complete_count = result.references.iter()
...
fn evaluate_weights(papers: &[PaperData], weights: &ScoringWeights) -> (f64, f64) {
    ...
    for paper in papers {
        ...
        let best_f1_idx = paper.strategy_metrics
            .iter()
            .enumerate()
        ...
        // Find best by score
        let best_score_idx = paper.strategy_metrics
            .iter()
            .enumerate()
            .max_by(|a, b| {
...
fn has_extractable_content(raw_ref: &str, config: &PdfParsingConfig) -> bool {
    let (title, _) = hallucinator_pdf::title::extract_title_from_reference(raw_ref);
    let has_title = !title.is_empty() && title.split_whitespace().count() >= config.min_title_words();

    let authors = hallucinator_pdf::authors::extract_authors_from_reference(raw_ref);
...
fn compute_f1(extracted: &[String], ground_truth: &[String], config: &PdfParsingConfig) -> (f64, f64, f64) {
    ...
    let recall = if ground_truth.is_empty() { 0.0 } else { matches as f64 / ground_truth.len() as f64 };
